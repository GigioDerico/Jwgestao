  ...
  from (
    select
      ...
      jsonb_build_object(
        ...
        'president', case when p.id is null then null else jsonb_build_object('full_name', p.full_name, 'phone', p.phone) end,
        'opening_prayer', case when op.id is null then null else jsonb_build_object('full_name', op.full_name) end,
        'closing_prayer', case when cp.id is null then null else jsonb_build_object('full_name', cp.full_name) end,
        'treasure_talk_speaker', case when tts.id is null then null else jsonb_build_object('full_name', tts.full_name) end,
        'treasure_gems_speaker', case when tgs.id is null then null else jsonb_build_object('full_name', tgs.full_name) end,
        'treasure_reading_student', case when trs.id is null then null else jsonb_build_object('full_name', trs.full_name, 'phone', trs.phone) end,
        'cbs_conductor', case when cbsc.id is null then null else jsonb_build_object('full_name', cbsc.full_name) end,
        'cbs_reader', case when cbsr.id is null then null else jsonb_build_object('full_name', cbsr.full_name) end,
        'ministry_parts', coalesce((
          select jsonb_agg(
            jsonb_build_object(
              ...
              'student', case when sm.id is null then null else jsonb_build_object('full_name', sm.full_name, 'phone', sm.phone) end,
              'assistant', case when am.id is null then null else jsonb_build_object('full_name', am.full_name) end
            )
        ...
        'christian_life_parts', coalesce((
          select jsonb_agg(
            jsonb_build_object(
              ...
              'speaker', case when sp.id is null then null else jsonb_build_object('full_name', sp.full_name) end
            )
  ...
  from (
    select
      ...
      jsonb_build_object(
        ...
        'president', case when p.id is null then null else jsonb_build_object('full_name', p.full_name, 'phone', p.phone) end,
        'closing_prayer', case when cp.id is null then null else jsonb_build_object('full_name', cp.full_name) end,
        'watchtower_conductor', case when wc.id is null then null else jsonb_build_object('full_name', wc.full_name) end,
        'watchtower_reader', case when wr.id is null then null else jsonb_build_object('full_name', wr.full_name, 'phone', wr.phone) end
      ) as meeting_payload
